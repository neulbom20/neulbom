...
<script type="text/javascript">
 ...
 // }
 function check(){
		var form0922 = document.form2;
		
		if (form0922.board_title.value =="") {
			alert("제목을 적어주세요");
		    form0922.board_title.focus();
		    return false;
	
	//}  else if (form0922.content.value ==""){
	//	document.getElementById("lblName") .innerHTML = form0922.subject.value;
	
	}  else if (form0922.board_content.value =="") {
		alert("내용을 적어주세요");
		form0922.board_content.focus();
		return false;
		
	}  else if (form0922.board_writer.value =="") {
		alert("작성자를 입력하세요");
		form0922.board_writer.focus();
		return false; 
	}
		form0922.submit();
		return;
...
<body>
<h1>자유게시판 수정</h1>
<form name="form2" action="${pageContext.request.contextPath}/Spring/BoardModifyActionController" method="post">
...
<input type="hidden" name="board_idx" value="${bv.board_idx}"> 

<table cellpadding=0 cellspacing=1 border=1>
	<tr>
		<td align="center" class="titlecolor">제목</td>
		<td><input type="text" name="board_title" maxlength="50"></td>		
	</tr>			
	
	<tr>
		<td align="center" class="titlecolor">내용</td>
		<td>
		<textarea name="board_content" cols=100 rows=20></textarea>		
		</td>
	</tr>	
	
	<tr>
		<td align="center" class="titlecolor">작성자</td>
		<td><input type="text" name="board_writer" value="${board_idx}"></td>
	</tr>	
	...
	<tr>
		...
		<input type="button" name="button" value="목록" onclick="javascript:document.location.href='${pageContext.request.contextPath}/Spring/BoardListController?page=${scri.page}&searchType=${scri.searchType}&keyword=${keyword}'"/>
